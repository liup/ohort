...
def convertEmailToUser(email):
    try:
        user = User.objects.get(email__iexact=email)
    except User.DoesNotExist:
        user = User.objects.get(username=email)
        if not user.is_staff:
            raise forms.ValidationError(_("No such email registered. Please try again."))
    return user

def generateUsername(first, last):
    """generate user name base on first and last name and a random number
    """
    ...
    try:
        ...
        return username
    return first + '_' + last + '_' + str(int(time.time()) + int(random.uniform(1,100)))


class LoginForm(forms.Form):

    username = forms.CharField(label=_("Email"), widget=forms.TextInput())
    ...
    def clean(self):
        if self._errors:
            return
        try:
            tmpuser = convertEmailToUser(self.cleaned_data["username"])
        except User.DoesNotExist:
            raise forms.ValidationError(_("The username and/or password you specified are not correct."))
        user = authenticate(username=tmpuser.username, password=self.cleaned_data["password"])
        if user:
            if user.is_active:
                self.user = user
            else:
...
class SignupForm(forms.Form):
    username = forms.EmailField(label=_("Email"), required = True,
								help_text =_("This email is used for login"),
    ...
    company_email = forms.EmailField(
        label = _("Company/School Email"),
		help_text =_("This email is used for membership verification"),
        required = True,
        widget = forms.TextInput()
    )

    receive_with_company_email = forms.BooleanField(
    #    label = _("Use company/school email to receive email notifications "),
		label = _(" "), 
		help_text = _("Use company email to receive email notifications"),
        required = False,
    ...
    def clean_username(self):
        try:
            username = User.objects.get(email=self.cleaned_data["username"])
        except User.DoesNotExist:
    ...
    def save(self):
        ...
        receive_with_company_email = self.cleaned_data["receive_with_company_email"]
        ...
        if confirmed:
            ...
            if email:
                EmailAddress.objects.add_email(new_user, email)
            if company_email:
                new_user.message_set.create(message=ugettext(u"Confirmation email sent to %(email)s") % {'email': company_email})
                EmailAddress.objects.add_email(new_user, company_email)

        new_user.first_name = first_name
        new_user.last_name = last_name + '---' + zip;
        if receive_with_company_email:
            new_user.email = company_email
        else:
